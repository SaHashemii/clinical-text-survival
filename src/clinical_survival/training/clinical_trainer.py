"""
Training loops for clinical unimodal Cox models.
"""
...
from clinical_survival.training.cross_validation import event_aware_batch_indices
from clinical_survival.training.losses import cox_ph_loss
from clinical_survival.training.metrics import concordance_index
...
def _make_batches(
    ...
    if training_style in {"random_batch", "clinical_batch"}:
...
def train_clinical_cox(
    ...
    *,
    ...
    """Train a clinical-only Cox model."""
...
def evaluate_clinical_cox(
    model: nn.Module,
    tensors: TensorTensors,
    sample_ids: list[str],
) -> tuple[float, pd.DataFrame]:
    """Evaluate a clinical-only Cox model and return risk scores."""
